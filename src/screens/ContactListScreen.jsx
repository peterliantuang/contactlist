...
const ContactListScreen = ({navigation}) => {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      if (contacts.length < 15) {
         console.log('--- contact lenght: '+contacts.length)
        for (let i = 1; i <= 1; i++) {
          dispatch(addContact({
            id: uuid.v4(),
            name: getRandomName(),
  ...
  }, [navigation, dispatch, isSearchFocused, contacts.length]);
    ...
    <>
      ...
      {!isSearchFocused && (
        <View style={styles.contactItem}>
          <Image
            source={require('../assets/user1.webp')}
            style={styles.profileIcon}
          />
          <View style={styles.contactInfo}>
            <Text style={[styles.contactName, styles.myName]}>
              Peter Gangmei
            </Text>
